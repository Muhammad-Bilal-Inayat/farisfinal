...
const getVehicleImage = (name: string) => {
  ...
  if (n.includes('camry') || n.includes('taurus') || n.includes('sedan')) {
    return 'https://images.unsplash.com/photo-1549317661-bd32c8ce0db2?auto=format&fit=crop&w=600&q=60&fm=webp';
  } else if (n.includes('mercedes') || n.includes('lexus') || n.includes('s-class')) {
    return 'https://images.unsplash.com/photo-1563720223185-11003d516935?auto=format&fit=crop&w=600&q=60&fm=webp';
  } else if (n.includes('gmc') || n.includes('yukon') || n.includes('suv')) {
    return 'https://images.unsplash.com/photo-1533473359331-0135ef1b58bf?auto=format&fit=crop&w=600&q=60&fm=webp';
  } else if (n.includes('staria') || n.includes('van') || n.includes('hiace') || n.includes('h1')) {
    return 'https://images.unsplash.com/photo-1621007947382-bb3c399b52c5?auto=format&fit=crop&w=600&q=60&fm=webp';
  } else if (n.includes('bus') || n.includes('coaster')) {
    return 'https://images.unsplash.com/photo-1570125909232-eb263c188f7e?auto=format&fit=crop&w=600&q=60&fm=webp';
  }
  return 'https://images.unsplash.com/photo-1494976388531-d1058494cdd8?auto=format&fit=crop&w=600&q=60&fm=webp';
};
...
export default function Home() {
  ...
  // Auto carousel effect changing background images every 5 seconds
  useEffect(() => {
  ...
  return (
    <div className="bg-white text-[var(--color-dark-charcoal)] flex flex-col">
        ...
        <div className="container relative mx-auto px-4 sm:px-6 lg:px-8 xl:px-10 z-10">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 lg:gap-6 xl:gap-8 items-center">
            ...
            >
              ...
              <VisualText as="h1" id="hero_title" className="text-2xl sm:text-3xl lg:text-3xl xl:text-4xl 2xl:text-5xl font-black leading-tight sm:leading-[1.15] text-white mb-2 sm:mb-2.5 xl:mb-3 tracking-tight">{t('hero_title')}</VisualText>
              <p className="text-xs sm:text-sm lg:text-sm xl:text-base 2xl:text-lg text-gray-200 mb-3 sm:mb-4 xl:mb-5 max-w-xl xl:max-w-2xl leading-relaxed">
                {t('hero_subtitle')}
              ...
              <div className="grid grid-cols-3 gap-2 sm:flex sm:gap-3 xl:gap-5 max-w-md mb-3 sm:mb-4">
                <div className="bg-white/10 backdrop-blur-md p-2 sm:py-2 sm:px-3 rounded-xl border border-white/10 shadow-xs flex flex-col text-center">
                  <span className="text-base sm:text-xl xl:text-2xl font-extrabold text-white leading-none">24/7</span>
                  <span className="text-[10px] sm:text-xs uppercase tracking-wider text-gray-300 font-bold mt-0.5">{t('support_24_7')}</span>
                </div>
                <div className="bg-white/10 backdrop-blur-md p-2 sm:py-2 sm:px-3 rounded-xl border border-white/10 shadow-xs flex flex-col text-center">
                  <span className="text-base sm:text-xl xl:text-2xl font-extrabold text-white leading-none">100%</span>
                  <span className="text-[10px] sm:text-xs uppercase tracking-wider text-gray-300 font-bold mt-0.5">{t('private_rides')}</span>
                </div>
                <div className="bg-white/10 backdrop-blur-md p-2 sm:py-2 sm:px-3 rounded-xl border border-white/10 shadow-xs flex flex-col text-center">
                  <span className="text-base sm:text-xl xl:text-2xl font-extrabold text-white leading-none">2026</span>
                  <span className="text-[10px] sm:text-xs uppercase tracking-wider text-gray-300 font-bold mt-0.5">{t('vip_fleet_badge')}</span>
                </div>
              </div>
            ...
            <motion.div 
              ...
              className="lg:col-span-6 xl:col-span-5 flex justify-center w-full mt-3 lg:mt-0" 
              id="booking"
            >
              <div className="w-full max-w-lg lg:max-w-none xl:max-w-lg bg-white/98 backdrop-blur-md p-3.5 sm:p-5 xl:p-6 border border-gray-200 shadow-xl rounded-2xl relative">
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-[var(--color-saudi-green)] text-white px-3.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-widest whitespace-nowrap shadow-md">
                  {t('instant_fare_booking')}
                </div>
                <div className="pt-1">
                  <Suspense fallback={<div>Loading</div>}><BookingWidget /></Suspense>
                </div>
              </div>
            </motion.div>
      ...
      <VisualSection id="fleet_specs_section" style={getSectionStyle('fleet_specs', 2)} className="py-10 md:py-14 bg-[#F1F4F8] border-b border-slate-200/80">
  ...
  <div className="absolute inset-0 bg-gradient-to-t from-black/75 via-black/20 to-transparent flex flex-col justify-end p-5 text-white">
    <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold bg-[var(--color-luxury-gold)] text-[var(--color-dark-charcoal)] w-fit mb-2 shadow-sm">
      <Sparkles size={12} /> {t('direct_haram_transfers')}
    </div>
    <h3 className="text-lg sm:text-xl font-bold">
      {isAr ? 'مطار جدة ➔ مكة المكرمة والمدينة المنورة' : 'Jeddah Airport ➔ Makkah & Madinah'}
    </h3>
    <p className="text-lg md:text-base text-emerald-100 opacity-90">{t('door_to_door_desc')}</p>
  </div>
</div>
            ...
            >
              <span className="text-xs uppercase font-bold tracking-[0.2em] text-[var(--color-saudi-green)] block mb-1">
                {t('intercity_transfers')}
              </span>
              <h2 className="text-2xl sm:text-3xl font-extrabold text-[var(--color-dark-charcoal)] mb-3 leading-tight">
                {t('transparent_rates_title')}
              </h2>
              <p className="text-lg md:text-base text-[var(--color-dark-charcoal)]/70 mb-4 leading-relaxed">
              ...
              <div className="space-y-2.5 mb-5">
                <Link 
                  ...
                  className="p-3 bg-white hover:bg-emerald-50/70 transition-all rounded-xl border border-slate-200/90 hover:border-emerald-300 flex items-center justify-between group shadow-xs cursor-pointer block"
                >
                  <div className="flex items-center gap-2.5">
                    <div className="w-8 h-8 rounded-lg bg-[var(--color-saudi-green)] text-white flex items-center justify-center font-bold text-xs shrink-0 group-hover:scale-105 transition-transform">
                      1
                    </div>
                    <div>
                      <h4 className="text-sm sm:text-base font-bold text-[var(--color-dark-charcoal)] group-hover:text-[var(--color-saudi-green)] transition-colors">
                  ...
                  <div className="text-right rtl:text-left shrink-0 pl-2 rtl:pr-2">
                    <span className="text-sm sm:text-base font-extrabold text-[var(--color-saudi-green)] block whitespace-nowrap">200 – 300 {t('sar')}</span>
                ...
                >
                  ...
                  <div className="text-right rtl:text-left shrink-0 pl-2 rtl:pr-2">
                    <span className="text-sm sm:text-base font-extrabold text-[var(--color-saudi-green)] block whitespace-nowrap">450 – 600 {t('sar')}</span>
                ...
                >
                  ...
                  <div className="text-right rtl:text-left shrink-0 pl-2 rtl:pr-2">
                    <span className="text-sm sm:text-base font-extrabold text-[var(--color-saudi-green)] block whitespace-nowrap">150 – 200 {t('sar')}</span>
              ...
              <div className="flex items-center gap-3">
                <Link 
                  to="/booking"
                  className="bg-[var(--color-saudi-green)] hover:bg-emerald-800 text-white px-5 py-3 rounded-xl font-bold text-xs uppercase tracking-wider transition-colors shadow-md flex items-center gap-1.5 cursor-pointer"
                >
                  {t('book_instant_transfer')} <ArrowRight size={14} className="rtl:rotate-180" />
                </Link>
                <a 
                  href="https://wa.me/966576124752?text=Assalamu%20Alaikum!%20I%20would%20like%20to%20inquire%20about%20Umrah%20transfers."
                  target="_blank" 
                  rel="noopener noreferrer"
                  className="text-sm font-bold text-[var(--color-saudi-green)] hover:underline flex items-center gap-1.5"
                >
                  <PhoneCall size={15} /> {t('whatsapp_dispatch')}
                </a>
              </div>
            </motion.div>
      ...
      <VisualSection id="routes_section" style={getSectionStyle('routes', 4)} className="py-10 md:py-14 bg-white border-b border-slate-200/80">
        <div className="container mx-auto px-4 sm:px-6 lg:px-10">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-10 items-center">
            ...
            >
              <div className="rounded-2xl overflow-hidden shadow-lg border border-slate-200/90 bg-white p-2">
                <ResponsiveImage width="1000" height="800" src={(parsedConfig && parsedConfig.find((s: any) => s.id === "fleet_specs")?.image) || "https://images.unsplash.com/photo-1618843479313-40f8afb4b4d8?auto=format&fit=crop&w=1000&q=80"} 
              ...
              <div className="grid grid-cols-3 gap-2 mt-3">
                <div className="bg-[#F1F4F8] p-2.5 rounded-xl border border-slate-200/80 text-center shadow-2xs">
                  <span className="text-sm md:text-xs text-gray-600 font-bold uppercase block">Mercedes S450</span>
                  <span className="text-lg md:text-base font-extrabold text-[var(--color-dark-charcoal)]">3 {t('pass')} • 3 {t('luggage_short')}</span>
                </div>
                <div className="bg-[#F1F4F8] p-2.5 rounded-xl border border-slate-200/80 text-center shadow-2xs">
                  <span className="text-sm md:text-xs text-gray-600 font-bold uppercase block">Hyundai Staria</span>
                  <span className="text-lg md:text-base font-extrabold text-[var(--color-dark-charcoal)]">7-9 {t('pass')} • 6 {t('luggage_short')}</span>
                </div>
                <div className="bg-[#F1F4F8] p-2.5 rounded-xl border border-slate-200/80 text-center shadow-2xs">
                  <span className="text-sm md:text-xs text-gray-600 font-bold uppercase block">Ford Taurus</span>
                  <span className="text-lg md:text-base font-extrabold text-[var(--color-dark-charcoal)]">4 {t('pass')} • 3 {t('luggage_short')}</span>
                </div>
              </div>
            </motion.div>
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-3 mb-5">
                <div className="p-3.5 bg-[#F1F4F8] rounded-xl border border-slate-200/90 flex items-start gap-2.5 shadow-2xs hover:border-emerald-200 transition-colors">
                  <Wifi size={16} className="text-[var(--color-saudi-green)] shrink-0 mt-0.5" />
                  <div>
                    <h4 className="text-lg md:text-base font-bold text-[var(--color-dark-charcoal)]">{t('high_speed_wifi')}</h4>
                    <p className="text-sm md:text-xs text-gray-600">{t('wifi_desc')}</p>
                  </div>
                </div>

                <div className="p-3.5 bg-[#F1F4F8] rounded-xl border border-slate-200/90 flex items-start gap-2.5 shadow-2xs hover:border-emerald-200 transition-colors">
                  <Coffee size={16} className="text-[var(--color-luxury-gold)] shrink-0 mt-0.5" />
                  <div>
                    <h4 className="text-lg md:text-base font-bold text-[var(--color-dark-charcoal)]">{t('zamzam_water')}</h4>
                    <p className="text-sm md:text-xs text-gray-600">{t('water_desc')}</p>
                  </div>
                </div>

                <div className="p-3.5 bg-[#F1F4F8] rounded-xl border border-slate-200/90 flex items-start gap-2.5 shadow-2xs hover:border-emerald-200 transition-colors">
                  <ShieldCheck size={16} className="text-[var(--color-saudi-green)] shrink-0 mt-0.5" />
                  <div>
                    <h4 className="text-lg md:text-base font-bold text-[var(--color-dark-charcoal)]">{t('licensed_drivers')}</h4>
                    <p className="text-sm md:text-xs text-gray-600">{t('licensed_drivers_desc')}</p>
                  </div>
                </div>

                <div className="p-3.5 bg-[#F1F4F8] rounded-xl border border-slate-200/90 flex items-start gap-2.5 shadow-2xs hover:border-emerald-200 transition-colors">
                  <Clock size={16} className="text-[var(--color-saudi-green)] shrink-0 mt-0.5" />
                  <div>
                    <h4 className="text-lg md:text-base font-bold text-[var(--color-dark-charcoal)]">{t('flight_tracking')}</h4>
                    <p className="text-sm md:text-xs text-gray-600">{t('flight_tracking_desc')}</p>
                  </div>
                </div>
              </div>
              ...
              <div>
                <Link 
                  to="/vehicles"
                  className="inline-flex items-center gap-1.5 bg-[var(--color-saudi-green)] hover:bg-[var(--color-saudi-emerald)] text-white px-5 py-2.5 rounded-xl font-bold text-xs uppercase tracking-wider transition-colors shadow-sm"
                >
                  {t('view_all_specs')} <ArrowRight size={14} className="rtl:rotate-180" />
                </Link>
              </div>
            </motion.div>
      ...
      <VisualSection id="ziyarat_section" style={getSectionStyle('ziyarat', 5)} className="py-10 md:py-14 bg-[#F1F4F8] border-b border-slate-200/80">
        <div className="container mx-auto px-4 sm:px-6 lg:px-10">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-10 items-center">
            ...
            >
              <div className="rounded-2xl overflow-hidden shadow-lg border border-slate-200/90 group">
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-black/75 via-black/20 to-transparent flex flex-col justify-end p-5 text-white">
                  <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold bg-white text-[var(--color-dark-charcoal)] w-fit mb-2 shadow-sm">
                    <Compass size={12} className="text-[var(--color-saudi-green)]" /> {isAr ? 'جولات المزارات الإسلامية' : 'Guided Sacred Tours'}
                  </div>
                  <h3 className="text-lg sm:text-xl font-bold">{t('historical_ziyarat_title')}</h3>
                  <p className="text-lg md:text-base text-emerald-100 opacity-90">
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-5">
                ...
                <div className="p-3.5 bg-white rounded-xl border border-slate-200/90 shadow-xs">
                  <h4 className="text-lg md:text-base font-bold text-[var(--color-dark-charcoal)] mb-1.5 flex items-center gap-1.5">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <Link
                  to="/ziyarat"
                  className="bg-[var(--color-luxury-gold)] hover:bg-amber-600 text-white px-5 py-2.5 rounded-xl font-bold text-xs uppercase tracking-wider transition-colors inline-flex items-center gap-2 shadow-md"
                >
                  <Compass size={14} /> {t('interactive_route_map')} <ArrowRight size={14} className="rtl:rotate-180" />
                </Link>
                <a 
                  href="https://wa.me/966576124752?text=Assalamu%20Alaikum!%20I%20would%20like%20to%20book%20a%20private%20Ziyarat%20tour."
                  target="_blank" 
                  rel="noopener noreferrer"
                  className="bg-[var(--color-saudi-green)] hover:bg-emerald-800 text-white px-5 py-2.5 rounded-xl font-bold text-xs uppercase tracking-wider transition-colors inline-flex items-center gap-2 shadow-md"
                >
                  {t('book_via_whatsapp')}
                </a>
              </div>
      ...
      <VisualSection id="vehicles_section" style={getSectionStyle('vehicles', 6)} className="py-10 md:py-14 bg-white border-b border-slate-200/80">
        <div className="container mx-auto px-4 sm:px-6 lg:px-10">
          <div className="text-center max-w-2xl mx-auto mb-8">
            <VisualText as="span" id="fleet_tag" className="text-xs uppercase font-bold tracking-[0.2em] text-[var(--color-saudi-green)] block mb-1">{t('premium_fleet')}</VisualText>
            <VisualText as="h2" id="fleet_title" className="text-3xl sm:text-4xl font-extrabold text-[var(--color-dark-charcoal)]">{t('explore_fleet')}</VisualText>
            <VisualText as="p" id="fleet_subtitle" className="mt-1.5 text-sm text-[var(--color-dark-charcoal)]/60 font-medium">{t('fleet_subtitle')}</VisualText>
          </div>
            ...
            {vehicles.map((v) => (
              <VisualDiv id={`vehicle_card_${v.id}`} key={v.id} className="bg-[#F8FAFC] hover:bg-white rounded-xl overflow-hidden shadow-xs border border-slate-200/90 hover:shadow-lg hover:border-emerald-300 transition-all duration-300 flex flex-col group">
                <Link to={`/routes-rates?vehicle=${v.id}`} className="block aspect-[16/10] relative overflow-hidden bg-gray-100 cursor-pointer">
                  ...
                  <div className="absolute top-3 right-3 bg-[var(--color-saudi-green)] text-white px-2.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider shadow-md">
                    {v.year || '2025'} {t('model')}
                  </div>
                </Link>
                <div className="p-4 sm:p-5 flex-grow flex flex-col">
                  <VisualText as="h3" id={`vehicle_title_${v.id}`} className="text-lg sm:text-xl font-bold text-[var(--color-dark-charcoal)] mb-1 uppercase tracking-wide">{v.name}</VisualText>
                  <div className="flex gap-4 mb-4 text-sm sm:text-base text-[var(--color-dark-charcoal)]/80 border-b border-slate-200/80 pb-3 font-semibold uppercase tracking-wider">
                  ...
                  <div className="flex items-center justify-between mt-auto">
                    <div>
                      <p className="text-xs text-[var(--color-luxury-gold)] uppercase tracking-widest font-bold">{t('starting_from')}</p>
                      <p className="text-2xl font-extrabold text-[var(--color-saudi-green)]">{v.startingPrice} <span className="text-sm font-normal text-[var(--color-dark-charcoal)]/60">{t('sar')}</span></p>
                    </div>
      ...
      <VisualSection id="testimonials_section" style={getSectionStyle('testimonials', 7)} className="py-10 md:py-14 bg-[#F1F4F8] border-t border-slate-200/80">
        <div className="container mx-auto px-4 sm:px-6 lg:px-10">
          <div className="text-center max-w-2xl mx-auto mb-8">
            <span className="text-xs uppercase font-bold tracking-[0.2em] text-[var(--color-saudi-green)] block mb-1">
              {t('why_choose_us')}
            </span>
            <h2 className="text-3xl sm:text-4xl font-extrabold text-[var(--color-dark-charcoal)]">
              {t('premium_choice')}
            </h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 mb-10">
            {[
              ...
              <VisualDiv id={`feature_card_${i}`} key={i} className="bg-white p-5 rounded-xl shadow-xs border border-slate-200/90 hover:shadow-md transition-shadow">
                <div className="w-8 h-8 bg-emerald-50 rounded-lg flex items-center justify-center mb-3 text-[var(--color-saudi-green)]">
          ...
          <div className="text-center max-w-2xl mx-auto mb-6">
            <span className="text-xs uppercase font-bold tracking-[0.2em] text-[var(--color-saudi-green)] block mb-1">{t('testimonials')}</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
            {testimonials.map((review, i) => (
              <VisualDiv id={`testimonial_card_${i}`} key={i} className="bg-white p-5 rounded-xl shadow-xs border border-slate-200/90 relative group hover:border-[var(--color-luxury-gold)]/60 transition-colors">
                <div className="text-[var(--color-luxury-gold)] mb-3 flex gap-0.5">
                ...
                <div className="flex items-center gap-2.5">
                  <div className="w-8 h-8 rounded-full bg-emerald-100 flex items-center justify-center text-[var(--color-saudi-green)] font-bold text-xs">
                  ...
                  <div>
                    ...
                    <p className="text-xs uppercase tracking-wider text-gray-500 font-semibold">{review.location}</p>
                  </div>
                </div>
              </VisualDiv>
            ))}
